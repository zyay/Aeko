...
const privacy = `LYAN PRIVACY POLICY
Effective: 15 September 2026

Lyan processes prompts in this browser session. The official Border Beam (npm: border-beam) and Orbkit SHDR-21 shader run locally in the page. This UI does not call a cloud inference API. See also the Android APK privacy text at github.com/zyay/Lyan.`

const terms = `LYAN TERMS OF USE
Effective: 15 September 2026

Software is provided as-is. Outputs are not professional advice. Border Beam is MIT (Jakub Antalik / Libraries.dev). SHDR-21 is from zzzzshawn/orbkit. Do not use Lyan to break the law.`

function replyFor(prompt: string) {
  return `Lyan, on-device in this tab.\n\nYou said: “${prompt}”\n\nComposer glow is the original border-beam package. The orb is official Shdr21 from zzzzshawn/orbkit, switching idle / thinking / speaking.`
}
...
export default function App() {
  ...
  if (page !== 'chat') {
    return (
      <main style={shell}>
        <header style={topBar}>
          <button type="button" style={ghost} onClick={() => setPage('chat')}>
            Back
          </button>
          <strong>Lyan</strong>
          <span />
        </header>
        <pre style={legal}>{page === 'privacy' ? privacy : terms}</pre>
      </main>
    )
  }
  ...
  return (
    <main style={shell}>
      <header style={topBar}>
        <button type="button" style={ghost} onClick={() => setPage('privacy')}>
          Privacy
        </button>
        <strong>Lyan</strong>
        <button type="button" style={ghost} onClick={() => setPage('terms')}>
          Terms
        </button>
      </header>
